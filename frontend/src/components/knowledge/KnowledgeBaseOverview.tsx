  ...
  const loadActiveTasks = async () => {
    try {
      ...
      const needsRestart = data.tasks.filter(
        (t: Task) => t.status === "failed",
      );
      if (
        needsRestart.length > 0 &&
        !sessionStorage.getItem(RESTART_DISMISS_KEY)
      ) {
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="bg-white/80 backdrop-blur-sm rounded-xl border border-white/40 overflow-hidden"
              >
              ...
              >
                ...
                <div className="p-4 pb-3">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1.5 ml-2 flex-shrink-0">
                      ...
                      <div
                        className="relative"
                        ref={menuOpenId === collection.id ? menuRef : undefined}
                      >
                        <button
                ...
                <div className="border-t border-white/40 px-4 py-3 bg-gray-50/50 flex space-x-2">
                  ...
                  <button
                    ...
                    disabled={
                      !collection.needs_reindex ||
                      reindexingId === collection.id
                    }
                    className={clsx(
                  ...
                  >
                    <ArrowPathIcon
                      className={clsx(
                        "w-3 h-3",
                        reindexingId === collection.id && "animate-spin",
                      )}
                    />
                    <span>
                      {reindexingId === collection.id ? "索引中" : "重新索引"}
                    </span>
                  </button>
                </div>
              </div>
